...
При создании шаблона можно указать данные, встроенные `AdaptiveCard` в полезные данные, или во время выполнения с помощью [пакетов SDK для шаблонов](sdk.md).

## <a name="specify-data-within-the-card"></a>Указание данных в карточке

Чтобы предоставить данные непосредственно в полезных данных карты, просто добавьте `$data` атрибут `AdaptiveCard` в (см. ниже).

## <a name="binding-to-the-data"></a>Привязка к данным

Можно выполнить привязку к данным `body` в или `actions` на карте.

* Синтаксис привязки начинается с `{` и заканчивается на `}`. Например,`{myProperty}`
...
[![Эскиз](https://user-images.githubusercontent.com/1432195/53214462-88d46980-3601-11e9-908d-253a1bb940a8.png)](http://vnext.adaptivecards.io/designer)
...
Чтобы назначить "контекст данных" для любого элемента, добавьте `$data` атрибут к элементу.
...
* Если `$data` свойству Objects присвоено значение **массива**, то **сам объект будет повторяться для каждого элемента в массиве.** 
* При повторении, `$data` используемом в привязках свойств, областью действия является **отдельный элемент** в массиве.

Например, `TextBlock` ниже будет повторяться три раза, так как `$data` это массив. Обратите внимание `text` , что свойство привязано `name` к свойству отдельного объекта в массиве. 
...
Это ответ Azure DevOps, где `message` свойство является строкой, сериализованной в формате JSON. Чтобы получить доступ к значениям в строке, необходимо использовать `JSON.parse` функцию в нашем шаблоне.
...
Чтобы удалить весь элемент при выполнении условия, используйте `$when` свойство. Если `$when` результат вычисления `false` элемента не будет отображаться для пользователя.
