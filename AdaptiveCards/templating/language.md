...
* Если свойство `$data` элемента адаптивной карты привязано к **массиву**, то **сам элемент будет повторяться для каждого элемента в массиве.** 
* Любые выражения привязки (`{myProperty}`), используемые в значениях свойств, будут ограничены **отдельным элементом** в массиве.
* При привязке к массиву строк используйте `{$data}` для доступа к отдельному строковому элементу. Например, `"text": "{$data}"`
